...
### Local word-accurate Reddit captions

Reddit narration is generated by OpenRouter. To time its yellow caption
highlight against the final audio, this project uses a local `whisper.cpp`
speech-recognition pass — it does **not** replace OpenRouter TTS or make a paid
API call.

On macOS, install the small native CLI once, then download its English model:
...
The one-time `base.en` model is about 142 MB and lives at
`server/storage/whisper/ggml-base.en.bin`, which is already gitignored. Add this
to `server/.env.local` and restart the dev server:

```env
WHISPER_ALIGNMENT_ENABLED=true
```

Docker includes the same CLI and model automatically. The alignment audio and
JSON files are temporary and removed after every reel.
